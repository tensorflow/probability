...
def matvecmul(a, b, transpose_a=False, validate_args=False, name=None):
  ...
  with tf.name_scope(name, 'matvecmul', [a, b]):
    ...
    with tf.control_dependencies(
        maybe_check_arguments_are_matrix_and_vector(a, b, validate_args)):
      a = tf.identity(a)
      b = tf.identity(b)
...
def maybe_check_arguments_are_matrix_and_vector(matrix, vector, validate_args):
  """Validate that the dimensions of matrix and vector line up."""
  if matrix.shape.ndims is not None and vector.shape.ndims is not None:
    if matrix.shape.ndims != vector.shape.ndims + 1:
      raise ValueError('Inputs must be a (possibly batched) matrix '
                       'and a similarly batched vector.')
  elif validate_args:
    return [
        tf.assert_rank(
            matrix,
            rank=tf.rank(vector) + 1,
            message=('Inputs must be a (possibly batched) matrix '
                     'and a similarly batched vector.')),
    ]
  return []
