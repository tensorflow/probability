...
class InterpRegular1DGridTest(tf.test.TestCase):
  ...
  def test_matrix_interpolation(self):
    ...
    x = np.array([0., 0.5, 1.]).reshape(3, 1, 1)
    y = tfp.math.interp_regular_1d_grid(
    ...
    self.assertAllClose(self.evaluate(y_0p5)[0], 0.5 * mat_0 + 0.5 * mat_1)
  ...
  def _check_sinusoid(self,
                      ...
                      broadcast_y_ref=True,
                      dtype=np.float32):
    """Check correctness on a sinusoidal function."""
    p_axis = axis if axis >= 0 else axis + len(y_ref_shape)

    x_ref_min = 0.
    x_ref_max = 2 * np.pi

    # Create implied_x_ref, which has same shape = y_ref_shape.
    implied_x_ref = np.linspace(
        x_ref_min, x_ref_max, y_ref_shape[axis], dtype=dtype)
    for _ in y_ref_shape[p_axis + 1:]:
      implied_x_ref = implied_x_ref[..., np.newaxis]
    for _ in y_ref_shape[:p_axis]:
      implied_x_ref = implied_x_ref[np.newaxis, ...]

    y_ref = np.sin(implied_x_ref)
    if broadcast_y_ref:
      y_ref = y_ref + np.zeros(y_ref_shape, dtype=dtype)

    # x.shape[axis] can be anything, so use 20.
    x_shape = y_ref_shape[:p_axis] + (20,) + y_ref_shape[p_axis + 1:]

    # make sure some values are not in [x_ref_min, x_ref_max]
    x = np.linspace(
        x_ref_min - 0.01, x_ref_max + 0.01, np.prod(x_shape),
    ...
    with self.cached_session():
      y = tfp.math.interp_regular_1d_grid(
          ...
          axis=axis)
      self.assertAllEqual(x_shape, y.shape)
      self.assertAllClose(self.evaluate(y), y_expected, atol=0.05, rtol=0)

  def test_axis_1_shape_2_50_3_4_fvextrapolate_yesbcastyref_32bit(self):
  ...
  def test_axis_1_shape_2_50_3_4_fvextrapolate_nobcastyref_64bit(self):
    self._check_sinusoid(
        y_ref_shape=(2, 50, 3, 4),
        axis=1,
        broadcast_y_ref=False,
        fill_value='extrapolate',
        dtype=np.float32)

  def test_axis_n2_shape_2_3_50_4_fvextrapolate_nobcastyref_64bit(self):
    self._check_sinusoid(
  ...
  def test_axis_n1_shape_2_3_4_50_fvextrapolate_yesbcastyref_32bit(self):
    self._check_sinusoid(
  ...
  def test_axis_3_shape_2_3_4_50_fvmixed_yesbcastyref_32bit(self):
    self._check_sinusoid(
        y_ref_shape=(2, 3, 4, 50),
        axis=3,
        broadcast_y_ref=True,
        fill_value='extrapolate',
        fill_value_below='constant_extension',
        dtype=np.float32)
