...
class ExpectationTest(test_util.TestCase):
  ...
  def test_works_correctly(self):
    ...
    def e_fx_reparam(x):
      ...
      return tfp.monte_carlo.expectation(f, samples, p.log_prob)
    ...
    efx_true, efx_true_grad = tfp.math.value_and_gradient(
        lambda x: x, x)
    efx_reparam, efx_reparam_grad = tfp.math.value_and_gradient(
        e_fx_reparam, x)
    efx_score, efx_score_grad = tfp.math.value_and_gradient(
        e_fx_score, x)
  ...
  def test_docstring_example_normal(self):
    ...
    approx_kl_, approx_kl_grad = tfp.math.value_and_gradient(
        approximate_kl_normal_normal, mu_p, mu_q)
    exact_kl_, exact_kl_grad = tfp.math.value_and_gradient(
