...
class LinearOperatorLowerTriangular(linear_operator.LinearOperator):
  ...
  * If `is_X == None` (the default), callers should have no expectation either
  ...
  def _check_tril(self, tril):
    ...
    if tensor_shape.TensorShape(tril.shape).ndims is not None and tensor_shape.TensorShape(tril.shape).ndims < 2:
      raise ValueError(
  ...
  def _get_tril(self):
    ...
    return _linalg.band_part(self._tril, -1, 0)

  def _get_diag(self):
    """Gets the diagonal part of `tril` kwarg."""
    return _linalg.diag_part(self._tril)
