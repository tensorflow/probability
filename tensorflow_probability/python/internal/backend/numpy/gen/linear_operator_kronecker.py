  ...
  def _solve_matmul_internal(
    ...
    for o in reversed(self.operators):
      ...
      if ops.get_static_value(operator_dimension) is not None:
        ...
        if tensor_shape.TensorShape(output.shape)[-2] is not None and tensor_shape.TensorShape(output.shape)[-1] is not None:
          dim = int(tensor_shape.TensorShape(output.shape)[-2] * output_shape[-1] // operator_dimension)
      else:
        dim = _ops.cast(
