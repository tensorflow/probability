  ...
  for slc in slices:
    if slc is Ellipsis:  # Switch over to negative indexing.
      ...
      num_remaining_non_newaxis_slices = sum(
          s is not _ops.newaxis for s in slices[
              slices.index(Ellipsis) + 1:])
      idx = -num_remaining_non_newaxis_slices
    elif slc is _ops.newaxis:
      pass
    else:
...
def _slice_single_param(
  ...
  if (ops.get_static_value(array_ops.size(batch_shape)) != 0 and
      ops.get_static_value(array_ops.size(param_batch_shape)) == 0):
    return param
  ...
  if param_ndims_to_matrix_ndims > 0:
    if Ellipsis not in [
        slc for slc in slices if not ops.is_tensor(slc)]:
      param_slices.append(Ellipsis)
    param_slices = param_slices + [slice(None)] * param_ndims_to_matrix_ndims
