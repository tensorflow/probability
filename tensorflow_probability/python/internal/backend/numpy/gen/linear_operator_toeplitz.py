  ...
  * If `is_X == None` (the default), callers should have no expectation either
  ...
  def _check_row_col(self, row, col):
    ...
    for name, tensor in [["row", row], ["col", col]]:
      if tensor_shape.TensorShape(tensor.shape).ndims is not None and tensor_shape.TensorShape(tensor.shape).ndims < 1:
        raise ValueError("Argument {} must have at least 1 dimension.  "
                         "Found: {}".format(name, tensor))

    if tensor_shape.TensorShape(row.shape)[-1] is not None and tensor_shape.TensorShape(col.shape)[-1] is not None:
      if tensor_shape.TensorShape(row.shape)[-1] != tensor_shape.TensorShape(col.shape)[-1]:
        raise ValueError(
  ...
  def _shape(self):
    ...
    v_shape = _ops.broadcast_static_shape(
        tensor_shape.TensorShape(self.row.shape), tensor_shape.TensorShape(self.col.shape))
