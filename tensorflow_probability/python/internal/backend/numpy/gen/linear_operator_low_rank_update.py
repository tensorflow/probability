...
class LinearOperatorLowRankUpdate(linear_operator.LinearOperator):
  ...
  * If `is_X == None` (the default), callers should have no expectation either
  ...
  def _get_uv_as_tensors(self):
    ...
    u = ops.convert_to_tensor(self.u)
    if self.v is self.u:
      v = u
    else:
      v = ops.convert_to_tensor(self.v)
    return u, v
  ...
  def _determinant(self):
    ...
    #   C := D^{-1} + V^H L^{-1} U
    u, v = self._get_uv_as_tensors()
    det_c = _linalg.det(self._make_capacitance(u=u, v=v))
  ...
  def _make_capacitance(self, u, v):
    ...
    # V^H L^{-1} U
    vh_linv_u = _linalg.matmul(v, linv_u, adjoint_a=True)
