...
class _BaseLinearOperatorCirculant(linear_operator.LinearOperator):
  ...
  def block_shape_tensor(self):
    ...
    # If tensor_shape.TensorShape(spectrum.shape) = [s0, s1, s2], and block_depth = 2,
    # block_shape = [s1, s2]
  ...
  def block_shape(self):
    return tensor_shape.TensorShape(self.spectrum.shape)[-self.block_depth:]
  ...
  def _vectorize_then_blockify(self, matrix):
    ...
    if (tensor_shape.TensorShape(vec.shape).is_fully_defined() and
  ...
  def _shape(self):
    s_shape = tensor_shape.TensorShape(self._spectrum.shape)
    # Suppose tensor_shape.TensorShape(spectrum.shape) = [a, b, c, d]
    # block_depth = 2
    # Then:
    #   batch_shape = [a, b]
    #   N = c*d
...
class LinearOperatorCirculant(_BaseLinearOperatorCirculant):
  ...
  Fourier transforms.  Here we consider `tensor_shape.TensorShape(A.shape) = [N, N]` and ignore batch
  ...
  * This operator is positive definite if and only if `Real{H} > 0`.
  ...
  Suppose `tensor_shape.TensorShape(H.shape) = [B1,...,Bb, N]`.  We say that `H` is a Hermitian spectrum
...
class LinearOperatorCirculant2D(_BaseLinearOperatorCirculant):
  ...
  If `tensor_shape.TensorShape(H.shape) = [N0, N1]`, (`N0 * N1 = N`):
  ...
  * This operator is positive definite if and only if `Real{H} > 0`.
  ...
  Suppose `tensor_shape.TensorShape(H.shape) = [B1,...,Bb, N0, N1]`, we say that `H` is a Hermitian
...
class LinearOperatorCirculant3D(_BaseLinearOperatorCirculant):
  ...
  If `tensor_shape.TensorShape(H.shape) = [N0, N1, N2]`, (`N0 * N1 * N2 = N`):
  Loosely speaking, matrix multiplication is equal to the action of a
  ...
  * This operator is positive definite if and only if `Real{H} > 0`.
  ...
  Suppose `tensor_shape.TensorShape(H.shape) = [B1,...,Bb, N0, N1, N2]`, we say that `H` is a Hermitian
