...
class RunningStatsTest(test_util.TestCase):
  ...
  def test_higher_rank_running_variance(self):
    ...
    final_mean, final_var = self.evaluate([
        state.mean,
        running_var.finalize(state)])
    self.assertAllClose(np.mean(x, axis=0), final_mean, rtol=1e-6)
  ...
  def test_running_covariance_as_variance(self):
    ...
    self.assertAllClose(final_cov, final_var, rtol=1e-6)
  ...
  def test_zero_running_covariance(self):
    ...
    self.assertAllClose([0., 0.], final_mean, rtol=1e-6)
  ...
  def test_running_covariance(self, ddof):
    ...
    self.assertAllClose(np.mean(x, axis=0), final_mean, rtol=1e-6)
    self.assertAllClose(np.cov(x.T, ddof=ddof), final_cov, rtol=1e-6)
  ...
  def test_running_covariance_with_event_ndims(self):
    ...
    manual_cov /= 300
    self.assertAllClose(manual_cov, final_cov, rtol=1e-6)
  ...
  def test_batched_running_covariance(self):
    ...
    for i in range(x_reshaped.shape[1]):
      ...
      self.assertAllClose(np_cov, final_cov[i], rtol=1e-6)
  ...
  def test_shift_in_running_covariance(self):
    ...
    self.assertAllClose(final_cov, np.cov(x.T, ddof=0), rtol=1e-6)
    self.assertAllClose(
        final_shifted_cov, np.cov(x.T, ddof=0), rtol=1e-2)
  ...
  def test_tf_while(self):
    ...
    self.assertAllClose(final_cov, np.cov(x.T, ddof=0), rtol=1e-6)
    self.assertAllClose(final_var, np.var(x, axis=0), rtol=1e-6)
