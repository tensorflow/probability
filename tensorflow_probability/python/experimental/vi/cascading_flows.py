...
from tensorflow_probability.python.experimental.bijectors import HighwayFlow
from tensorflow_probability.python.experimental.bijectors import build_highway_flow_layer
...
CFParameters = collections.namedtuple('CFParameters', ['cascading_flows'])
...
def _cf_convex_update_for_base_distribution(dist,
    ...
    if variables is None:
        ...
        bijectors.append(build_highway_flow_layer(tf.reduce_prod(actual_event_shape),
                                                  activation_fn=None))

        variables = [b.parameters for b in bijectors[1:]]

    else:

        for i in range(0, layers):
            bijectors.append(HighwayFlow(**variables[i]))
