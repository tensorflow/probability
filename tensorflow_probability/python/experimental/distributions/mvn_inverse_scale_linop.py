...
class MultivariateNormalInverseScaleLinearOperator(
  ...
  `precision` matrix `P` has particular structure. For example, if
  ...
  * `||y||**2` denotes the squared Euclidean norm of `y`.
  ...
  def __init__(self,
    ...
    inverse covariance is known, the user may initialize this distribution
    with `tf.linalg.cholesky(inverse_covariance)`.
  ...
  def _log_prob(self, value):
    ...
    Returns:
      Floating point `Tensor` with batch shape.
    """
    return (-.5 * ps.reduce_sum(self.event_shape) * tf.math.log(2 * np.pi) +
  ...
  def _sample_n(self, n, seed=None):
    ...
    `inverse_scale` is a `tf.linalg.LinearOperatorLowerTriangular`, then
    sampling costs one matrix multiply.
