...
class PreconditionedHamiltonianMonteCarlo(HamiltonianMonteCarlo):
  ...
  momentum_distribution = tfp.experimental.distributions.MVNInverseScaleLinOp(
      tf.zeros(3),
  ...
  run_chain()  # [1000, 1000, 1000]
  ```

  ##### Estimate parameters of a more complicated posterior.

  This demonstrates using multiple state parts, and reshaping a
  `tfp.experimental.distributions.MVNInverseScaleLinOp` to use with a scalar
  or a non-square shape (in this case, `[2, 3, 4]`).
  ...
  momentum_distribution = tfd.JointDistributionSequential([
      ...
      tfd.Normal(0., 0.1),
      reshape_to_234(
          tfp.experimental.distributions.MVNInverseScaleLinOp(
  ...
  run_chain()  # [1000, 1000, 1000 * tf.ones([2, 3, 4])]
  ```
  """
  ...
  def one_step(self, current_state, previous_kernel_results, seed=None):
    with tf.name_scope(mcmc_util.make_name(self.name, 'hmc', 'one_step')):
      ...
      [
          ...
          kinetic_energy_fn=momentum_distribution.log_prob)
      ...
      new_kernel_results = previous_kernel_results._replace(
          log_acceptance_correction=_compute_log_acceptance_correction(
              momentum_distribution.log_prob, current_momentum_parts,
              next_momentum_parts),
...
def _compute_log_acceptance_correction(momentum_log_prob,
                                       current_momentums,
                                       proposed_momentums,
                                       name=None):
  ...
  Args:
    momentum_log_prob: Python callable that computes the log probability of
      the momentum of the state part.
  ...
  with tf.name_scope(name or 'compute_log_acceptance_correction'):
    current_kinetic = momentum_log_prob(current_momentums)
    proposed_kinetic = momentum_log_prob(proposed_momentums)
