...
class _ParticleFilterTest(test_util.TestCase):
  def test_smc_squared(self):
      # TODO: Random walk where outer parameter is scale of the random walk
      # initial particles start from a bad position and quickly rejuvenated
      num_particles = 1024
      ...
      results = self.evaluate(
          particle_filter.smc_squared(
              ...
              initial_parameter_prior=normal.Normal(0., 1.),  # What would this be
              num_outer_particles=5,
              outer_rejuvenation_criterion_fn=lambda *_: False,
              inner_transition_fn=lambda _, state: normal.Normal(state, 10.),
              inner_observation_fn=lambda _, state: normal.Normal(state, 0.1),
