...
class FrechetCDFTest(test_util.TestCase):
  ...
  def testScalarCongruency(self):
    ...
    bijector_test_util.assert_scalar_congruency(
        tfb.FrechetCDF(loc=loc, scale=20., alpha=0.5), lower_x=1., upper_x=100.,
        eval_func=self.evaluate, rtol=0.05)

  def testBijectiveAndFinite(self):
    loc = np.array(-1., np.float64)
    bijector = tfb.FrechetCDF(loc=loc, scale=3.0, alpha=2., validate_args=True)
  ...
  def testVariablesScaleAndAlpha(self):
    x = tf.Variable(1.)
    y = tf.Variable(1.)
    b = tfb.FrechetCDF(loc=0., scale=x, alpha=y, validate_args=True)
    self.evaluate(x.initializer)
    self.evaluate(y.initializer)
    self.assertIs(x, b.scale)
    self.assertIs(y, b.alpha)
    self.assertEqual((), self.evaluate(b.forward(3.)).shape)
    with self.assertRaisesOpError("Argument `scale` must be positive."):
      with tf.control_dependencies([x.assign(-1.)]):
        self.evaluate(b.forward(-3.))
    with self.assertRaisesOpError("Argument `alpha` must be positive."):
