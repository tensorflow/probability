...
class Categorical(distribution.Distribution):
  ...
  def num_categories(self):
    ...
    with self._name_and_control_scope('num_categories'):
      # Note: its safe to use `convert_to_tensor` because either this call has
      # graph side-effects or returns a non-Tensor in which case
      # `convert_to_tensor` embeds the assertions. Ideally we'd use tf.identity
      # but cannot because of its failure to preserve static value; b/135200956.
  ...
  def logits_tensor(self, name=None):
    """Logits vector computed from non`None` input arg (`probs` or `logits`)."""
  ...
  def probs_tensor(self, name=None):
    """Probs vector computed from non`None` input arg (`probs` or `logits`)."""
  ...
  def log_probs(self, name=None):
    """Log-probs vector from non`None` input arg (`probs` or `logits`)."""
    with self._name_and_control_scope(name or 'probs_tensor'):
      if self.logits is None:
  ...
  def _batch_shape_tensor(self):
    x = self.probs if self.logits is None else self.logits
    return tf.shape(x)[:-1]
  ...
  def _sample_n(self, n, seed=None):
    logits = tf.math.log(self.probs) if self.logits is None else self.logits
  ...
  def _cdf(self, k):
    ...
    probs = self.probs if self.logits is None else tf.nn.softmax(self.logits)
  ...
  def _entropy(self):
    if self.logits is None:
      ...
      return -tf.reduce_sum(
          tf.math.multiply_no_nan(tf.math.log(self.probs), self.probs),
          axis=-1)
    m = tf.reduce_max(self.logits, axis=-1, keepdims=True)
    x = self.logits - m
    sum_exp_x = tf.reduce_sum(tf.math.exp(x), axis=-1)
    lse_logits = m[..., 0] + tf.math.log(sum_exp_x)
    return lse_logits - tf.reduce_sum(
        self.logits * tf.math.exp(x), axis=-1) / sum_exp_x
...
def _kl_categorical_categorical(a, b, name=None):
  ...
  with tf.name_scope(name or 'kl_categorical_categorical'):
    return tf.reduce_sum(tf.math.multiply_no_nan(a.probs_tensor(),
                                                 a.log_probs() - b.log_probs()),
                         axis=-1)
