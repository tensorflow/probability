...
class CauchyTest(test_util.TestCase):
  ...
  def testCauchyCauchyKLWithAnalytic(self):
    """
    Test the KL by comparing it against the analytic formula for it
    """
    batch_size = 6
    ...
    kl_expected = np.log((gamma_a + gamma_b)**2 + (mu_a - mu_b)**2) - np.log(4. * gamma_a * gamma_b)
  ...
  def testCauchyCauchyKLWithMC(self):
    """
    Test the KL by comparing it against its Monte Carlo estimate
    """
    batch_size = 6
    mu_a = np.array([3.0] * batch_size)
    gamma_a = np.array([1.0, 2.0, 3.0, 1.5, 2.5, 3.5])
