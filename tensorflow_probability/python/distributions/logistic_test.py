...
class LogisticTest(test_util.TestCase):
  ...
  def testReparameterizable(self):
    ...
    scale = 1.5
    dist = logistic.Logistic(loc, scale, validate_args=True)
    self.assertEqual(reparameterization.FULLY_REPARAMETERIZED,
                     dist.reparameterization_type)
  ...
  def testLogisticSample(self):
    loc_ = [3.0, 4.0, 2.0]
    scale_ = 1.0
    dist = logistic.Logistic(loc_, scale_, validate_args=True)
    n = int(15e3)
  ...
  def testDtype(self):
    ...
    dist64 = logistic.Logistic(loc, scale, validate_args=True)
  ...
  def testGradientThroughParams(self):
    ...
    scale = tf.Variable(2.)
    d = logistic.Logistic(loc=loc, scale=scale, validate_args=True)
  ...
  def testAssertsPositiveScale(self):
    ...
    with self.assertRaisesOpError("Argument `scale` must be positive."):
      d = logistic.Logistic(loc=0, scale=scale, validate_args=True)
