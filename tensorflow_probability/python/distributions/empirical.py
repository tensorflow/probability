...
def _broadcast_event_and_samples(event, samples, event_ndims):
  ...
  samples_shape = tf.shape(samples)[:-event_ndims-1]
...
class Empirical(distribution.Distribution):
  ...
  def __init__(self,
    ...
    Args:
      samples: Numeric `Tensor` of shape [B1, ..., Bk, S, E1, ..., En]`,
        ...
        distribution. n is specified by argument event_dims.
    ...
    with tf.name_scope(name, values=[samples]):
      ...
      with tf.control_dependencies(
        ...
        try:
          ...
          self._num_samples = samples_shape[-event_ndims-1]
        except IndexError:
          self._num_samples = tf.constant(1)
  ...
  def _batch_shape_tensor(self):
    return tf.shape(self.samples)[:-self._event_ndims-1]

  def _batch_shape(self):
    return self.samples.shape[:-self._event_ndims-1]

  def _event_shape_tensor(self):
    if self._event_ndims == 0:
      return tf.constant([], dtype=tf.int32)
    return tf.shape(self.samples)[-self._event_ndims:]

  def _event_shape(self):
    if self._event_ndims == 0:
      return tf.TensorShape([])
    return self.samples.shape[-self._event_ndims:]

  def _mean(self):
    return tf.reduce_mean(self.samples, axis=-self._event_ndims-1)

  def _stddev(self):
    r = self.samples - tf.expand_dims(self.mean(), axis=-self._event_ndims-1)
    var = tf.reduce_mean(tf.square(r), axis=-self._event_ndims-1)
    return tf.sqrt(var)

  def _sample_n(self, n, seed=None):
    indices = tf.random.uniform([n], maxval=self.num_samples,
                                dtype=tf.int32, seed=seed)
    draws = tf.gather(self.samples, indices, axis=-self._event_ndims-1)
    axes = tf.concat(
        [[tf.rank(self.samples)-self._event_ndims-1],
         tf.range(tf.rank(self.samples)-self._event_ndims-1),
         tf.range(self._event_ndims)+tf.rank(self.samples)-self._event_ndims],
        axis=0)
    draws = tf.transpose(draws, axes)
    return draws

  def _mode(self):
    def _get_mode(samples):
      # TODO: Swith to tf.unique_with_counts_v2 when updated in TensorFlow API
      count = gen_array_ops.unique_with_counts_v2(samples, axis=[0]).count
      mode = tf.gather(samples, tf.argmax(count))
      return mode
    ...
    mode = tf.map_fn(_get_mode, samples)
  ...
  def _cdf(self, event):
    ...
    cdf = tf.reduce_sum(
        tf.cast(samples-event <= 0, dtype=tf.int32),
        axis=-self._event_ndims-1) / self.num_samples
    return cdf
