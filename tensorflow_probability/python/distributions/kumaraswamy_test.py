...
class KumaraswamyTest(test_util.TestCase):
  ...
  def testSimpleShapes(self):
    a = np.random.rand(3)
    b = np.random.rand(3)
    dist = kumaraswamy.Kumaraswamy(a, b, validate_args=True)
  ...
  def testPdfAStretchedInBroadcastWhenLowerRank(self):
    ...
    b = [1., 2]
    x = [[.5, .5], [.2, .8]]
    pdf = kumaraswamy.Kumaraswamy(a, b, validate_args=True).prob(x)
    expected_pdf = _kumaraswamy_pdf(a, b, x)
    self.assertAllClose(expected_pdf, self.evaluate(pdf))
    self.assertEqual((2, 2), pdf.shape)
  ...
  def testKumaraswamyModeInvalid(self):
    with tf1.Session():
      a = np.array([1., 2, 3])
      b = np.array([2., 4, 1.2])
      dist = kumaraswamy.Kumaraswamy(
          a, b, allow_nan_stats=False, validate_args=True)
  ...
  def testKumaraswamyModeEnableAllowNanStats(self):
    with tf1.Session():
      ...
      b = np.array([2., 4, 1.2])
      dist = kumaraswamy.Kumaraswamy(
          a, b, allow_nan_stats=True, validate_args=True)
  ...
  def testKumaraswamySampleMultipleTimes(self):
    ...
    dist2 = kumaraswamy.Kumaraswamy(
        ...
        name='kumaraswamy2',
        validate_args=True)
    samples2 = self.evaluate(dist2.sample(n_val, seed=seed))
  ...
  def testKumaraswamyCdf(self):
    ...
    for dt in (np.float32, np.float64):
      ...
      actual = self.evaluate(
          kumaraswamy.Kumaraswamy(a, b, validate_args=True).cdf(x))
  ...
  def testKumaraswamyLogCdf(self):
    ...
    for dt in (np.float32, np.float64):
      ...
      x = np.random.random(shape).astype(dt)
      actual = self.evaluate(
          tf.exp(kumaraswamy.Kumaraswamy(a, b, validate_args=True).log_cdf(x)))
      self.assertAllEqual(np.ones(shape, dtype=np.bool_), 0. <= x)
      self.assertAllEqual(np.ones(shape, dtype=np.bool_), 1. >= x)
      self.assertAllClose(_kumaraswamy_cdf(a, b, x), actual)
