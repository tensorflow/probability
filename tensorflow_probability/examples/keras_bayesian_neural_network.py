...
def plot_weight_posteriors(names, qm_vals, qs_vals, fname):
  ...
  for n, qm in zip(names, qm_vals):
    sns.distplot(qm.numpy().flatten(), ax=ax, label=n)
  ...
  for n, qs in zip(names, qs_vals):
    sns.distplot(qs.numpy().flatten(), ax=ax)
...
def create_model():
  ...
  # calcuating the Evidence Lower Bound Loss (ELBO)
...
class MNISTSequence(tf.keras.utils.Sequence):
  """
  Class inheriting from the Keras Sequence class for producing
  a sequence of data (to be used with a model.(fit|evaluate|predict)_generator
  function).
  ...
  Args:
    ...
    fake_data: Boolean, for creating MNIST-like fake data.
  """
  def __init__(self, data=None, batch_size=128, size=None):
    if data:
      images, labels = data
    else:
  ...
  def __generate_fake_data(num_images, num_classes):
    '''
    Generate fake data in the shape of the MNIST dataset for unittest.
    Args:
  ...
  def __preprocessing(images, labels):
    '''
    Preprocessing for image and labels data.
