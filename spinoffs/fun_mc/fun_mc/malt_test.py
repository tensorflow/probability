...
class MALTTest(tfp_test_util.TestCase):
  ...
  def testPreconditionedMALT(self):
    ...
    _, chain = jax.jit(
        lambda state, seed: fun_mc.trace(  # pylint: disable=g-long-lambda
            state=(
                malt.metropolis_adjusted_langevin_trajectories_init(
  ...
  def testMALTNamedAxis(self):
    ...
    def kernel(state, seed):
      ...
      (malt_state, malt_extra) = (
          malt.metropolis_adjusted_langevin_trajectories_step(
              malt_state,
    ...
    self.assertAllClose(
        malt_state.state['shared'][0], malt_state.state['shared'][1]
    )
    self.assertTrue(
        np.any(
            np.abs(
                malt_state.state['sharded'][0] - malt_state.state['sharded'][1]
            )
            > 1e-3
        )
    )
    ...
    self.assertAllClose(
        malt_extra.log_accept_ratio[0], malt_extra.log_accept_ratio[1]
    )
