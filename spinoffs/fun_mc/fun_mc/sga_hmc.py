...
class HamiltonianMonteCarloWithStateGradsExtra(NamedTuple):
  ...
  num_integrator_steps: fun_mc.IntArray
...
def hamiltonian_monte_carlo_with_state_grads_step(
    ...
    trajectory_length: fun_mc.FloatArray,
    scalar_step_size: fun_mc.FloatArray,
    step_size_scale: float | fun_mc.FloatNest = 1.0,
  ...
  def hmc(*traj_and_flat_consts):
    ...
    scalar_step_size, hmc_state, step_size_scale = util.unflatten_tree(
        consts, traj_and_flat_consts[1:]
    )
    trajectory_length = jnp.asarray(trajectory_length)
    num_integrator_steps = jnp.asarray(
        jnp.ceil(trajectory_length / scalar_step_size), jnp.int32
    )
    # In case something goes negative.
    num_integrator_steps = jnp.maximum(1, num_integrator_steps)
    new_hmc_state, hmc_extra = fun_mc.hamiltonian_monte_carlo_step(
        hmc_state,
        num_integrator_steps=num_integrator_steps,
        step_size=util.map_tree(
            lambda s: s * scalar_step_size, step_size_scale
        ),
    ...
    def grad(*grads):
      ...
      def do_sum(x, named_axis):
        return distribute_lib.reduce_sum(
            x,
            list(range(len(trajectory_length.shape), len(x.shape))),
            named_axis,
        )
      ...
      traj_grad = sum(
          util.flatten_tree(
              util.map_tree_up_to(
                  state_grads, do_sum, state_grads, named_axis_bc
              )
          )
      )
...
def chees_criterion(
    ...
    trajectory_length: Optional[fun_mc.FloatArray] = None,
    state_mean: Optional[fun_mc.State] = None,
    state_mean_weight: float | fun_mc.FloatNest = 0.0,
    named_axis: Optional[fun_mc.StringNest] = None,
    chain_named_axis: Optional[fun_mc.StringNest] = None,
) -> tuple[fun_mc.FloatArray, fun_mc.FloatArray]:
  ...
  Args:
    previous_state: (Possibly nested) floating point `Array`. The previous state
      of the MCMC chain.
    proposed_state: (Possibly nested) floating point `Array`. The proposed state
      of the MCMC chain.
    accept_prob: Floating `Array`. Probability of acceping the proposed state.
    trajectory_length: Ignored.
    state_mean: (Possibly nested) floating point `Array`. Optional estimate of
      the MCMC chain mean.
    state_mean_weight: Floating point `Array`. Used to weight `state_mean` with
  ...
  state_mean_weight = fun_mc.maybe_broadcast_structure(
      state_mean_weight, previous_state
  )
  if named_axis is None:
  ...
  def _center_previous_state(x, mx, mw):
    ...
    return x - jax.lax.stop_gradient(x_center)
  ...
  def _center_proposed_state(x, mx, mw):
    ...
    x_center = distribute_lib.reduce_sum(
  ...
  def _square(x):
    return util.map_tree(jnp.square, x)

  def _sub(x, y):
    return util.map_tree(lambda x, y: x - y, x, y)

  previous_state = util.map_tree(
      _center_previous_state, previous_state, state_mean, state_mean_weight
  )
  proposed_state = util.map_tree(
      _center_proposed_state, proposed_state, state_mean, state_mean_weight
  )
  chees = 0.25 * jnp.square(
  ...
  chees = jnp.where(accept_prob > 1e-4, chees, 0.0)
...
class ChEESPerGradExtra(NamedTuple):
  chees: fun_mc.FloatArray
  per_chain_chees: fun_mc.FloatArray
  per_chain_chees_per_grad: fun_mc.FloatArray
...
def chees_per_grad_criterion(
    ...
    named_axis: Optional[fun_mc.StringNest] = None,
    chain_named_axis: Optional[fun_mc.StringNest] = None,
) -> tuple[fun_mc.FloatArray, ChEESPerGradExtra]:
  ...
  Args:
    ...
    power: Floating `Array`. Used to scale the `trajectory_length` term.
    state_mean: (Possibly nested) floating point `Array`. Optional estimate of
      the MCMC chain mean.
    state_mean_weight: Floating point `Array`. Used to weight `state_mean` with
      the mean computed by averaging across the previous/proposed state. Setting
  ...
  return (
      distribute_lib.reduce_mean(
          per_chain_chees_per_grad, named_axis=chain_named_axis
      ),
      extra,
  )
...
@util.named_call
def _halton(
    float_index: fun_mc.FloatArray, max_bits: float | fun_mc.FloatArray = 10
) -> fun_mc.FloatArray:
...
class DefaultTrajectoryLengthParams(NamedTuple):
  ...
  log_mean_trajectory_length: fun_mc.FloatArray

  @util.named_call
  def mean_trajectory_length(self) -> fun_mc.FloatArray:
    """Computes the mean trajectory length."""
    return jnp.exp(self.log_mean_trajectory_length)
...
def default_trajectory_length_sample(
  ...
  trajectory_length = (
      2
      * _halton(jnp.asarray(step, mean_trajectory_length.dtype))
...
def default_trajectory_length_constrain(
    trajectory_length_params: DefaultTrajectoryLengthParams,
    max_trajectory_length: float | fun_mc.FloatArray = 3.0,
) -> DefaultTrajectoryLengthParams:
...
def default_trajectory_length_init(
    init_trajectory_length: fun_mc.FloatArray,
) -> DefaultTrajectoryLengthParams:
  ...
  return DefaultTrajectoryLengthParams(
      log_mean_trajectory_length=jnp.log(init_trajectory_length)
  )
...
def stochastic_gradient_ascent_hmc_init(
    ...
    init_trajectory_length: fun_mc.FloatArray,
    trajectory_length_params_init_fn: Callable[
        [fun_mc.FloatArray], Any
    ] = default_trajectory_length_init,
):
  ...
  """
  init_trajectory_length = jnp.asarray(init_trajectory_length)
...
def stochastic_gradient_ascent_hmc_step(
    ...
    criterion_fn: Callable[
        [fun_mc.State, fun_mc.State, fun_mc.FloatArray, fun_mc.FloatArray],
        tuple[fun_mc.FloatArray, Any],
    ],
    trajectory_length_adaptation_rate: float | fun_mc.FloatArray = 0.05,
    trajectory_length_sample_fn: Callable[
        [Any, fun_mc.IntArray, Any], fun_mc.FloatArray
    ] = (default_trajectory_length_sample),
    ...
    adapt: bool | fun_mc.BooleanArray = True,
    seed: Any = None,
  ...
  def loss_fn(*args, **kwargs):
    ...
    trajectory_length = trajectory_length_sample_fn(
    ...
    accept_prob = jnp.exp(
        jnp.minimum(
            jnp.zeros_like(hmc_extra.hmc_extra.log_accept_ratio),
            hmc_extra.hmc_extra.log_accept_ratio,
        )
    )
    accept_prob = jnp.where(
        jnp.isfinite(accept_prob), accept_prob, jnp.zeros_like(accept_prob)
    )
  ...
  # Adapt trajectory.
  trajectory_length_params_opt_state, trajectory_length_params_opt_extra = (
      fun_mc.adam_step(
  ...
  (
      ...
      criterion,
      criterion_extra,
      trajectory_length_params,
  ) = trajectory_length_params_opt_extra.loss_extra

  # Constrain trajectory params.
  trajectory_length_params_opt_state = fun_mc.choose(
      adapt,
      trajectory_length_params_opt_state,
      sga_hmc_state.trajectory_length_params_opt_state,
  )
  ...
  trajectory_length_params_opt_state = (
      trajectory_length_params_opt_state._replace(
  ...
  trajectory_length_params_rmean_state = fun_mc.choose(
      adapt,
      trajectory_length_params_rmean_state,
      sga_hmc_state.trajectory_length_params_rmean_state,
  )
