...
def _polynomial_decay(
    step: fun_mc.AnyArray,
    step_size: fun_mc.FloatArray,
    decay_steps: fun_mc.AnyArray,
    final_step_size: fun_mc.FloatArray,
    power: float | fun_mc.FloatArray = 1.0,
) -> fun_mc.FloatArray:
  """Polynomial decay step size schedule."""
  step_size = jnp.asarray(step_size)
  step_f = jnp.array(step, step_size.dtype)
...
class StepSizeAdaptationState(NamedTuple):
  ...
  def step_size(self, num_adaptation_steps=None):
    if num_adaptation_steps is not None:
      return jnp.where(
          self.step < num_adaptation_steps,
          self.opt_step_size(),
          self.rms_step_size,
      )
    else:
...
class StepSizeAdaptationExtra(NamedTuple):
  ...
  accept_prob: fun_mc.FloatArray


@util.named_call
def step_size_adaptation_init(
    init_step_size: fun_mc.FloatArray,
) -> StepSizeAdaptationState:
  """Initializes `StepSizeAdaptationState`.

  Args:
    init_step_size: Floating point Array. Initial step size.

  Returns:
    step_size_adaptation_state: `StepSizeAdaptationState`
  """
  init_step_size = jnp.asarray(init_step_size)
  rms_state = fun_mc.running_mean_init(
      init_step_size.shape, init_step_size.dtype
  )
  rms_state = rms_state._replace(mean=init_step_size)

  return StepSizeAdaptationState(
      step=jnp.array(0, jnp.int32),
      opt_state=fun_mc.adam_init(jnp.log(init_step_size)),
      rms_state=rms_state,
  )
...
def step_size_adaptation_step(
    ...
    num_adaptation_steps: Optional[fun_mc.IntArray],
    target_accept_prob: float | fun_mc.FloatArray = 0.8,
    adaptation_rate: float | fun_mc.FloatArray = 0.05,
    adaptation_rate_decay_power: float | fun_mc.FloatArray = 0.1,
    averaging_window_steps: int | fun_mc.IntArray = 100,
    min_log_accept_prob: fun_mc.FloatArray = np.log(1e-5),
    reduce_fn: Callable[[fun_mc.FloatArray], fun_mc.FloatArray] = (
  ...
  target_accept_prob = jnp.asarray(target_accept_prob, dtype=dtype)
  adaptation_rate_decay_power = jnp.asarray(
      adaptation_rate_decay_power, dtype=dtype
  )
  min_log_accept_prob = jnp.full(
      log_accept_ratio.shape, jnp.array(min_log_accept_prob, dtype)
  )

  log_accept_prob = jnp.minimum(log_accept_ratio, jnp.zeros([], dtype))
  ...
  accept_prob = jnp.exp(reduce_fn(log_accept_prob))
  ...
  opt_state, opt_extra = fun_mc.adam_step(
      state.opt_state, loss_fn, adaptation_rate, **opt_kwargs
  )
  ...
  rms_state, _ = fun_mc.running_mean_step(
      old_rms_state,
      jnp.exp(opt_state.state),
      window_size=averaging_window_steps,
  )

  if num_adaptation_steps is not None:
    rms_state = util.map_tree(
        lambda n, o: jnp.where(state.step < num_adaptation_steps, n, o),
        rms_state,
        old_rms_state,
    )
...
class AdaptiveHamiltonianMonteCarloExtra(NamedTuple):
  ...
  step_size: fun_mc.FloatArray
  num_leapfrog_steps: fun_mc.IntArray
  mean_num_leapfrog_steps: fun_mc.IntArray

  @property
  def state(self) -> fun_mc.ArrayNest:
...
def adaptive_hamiltonian_monte_carlo_init(
    state: fun_mc.ArrayNest,
    target_log_prob_fn: fun_mc.PotentialFn,
    step_size: float | fun_mc.FloatArray = 1e-2,
    initial_mean: float | fun_mc.FloatNest = 0.0,
    initial_scale: float | fun_mc.FloatNest = 1.0,
    scale_smoothing_steps: int | fun_mc.IntArray = 10,
) -> AdaptiveHamiltonianMonteCarloState:
  ...
  running_var_state = running_var_state._replace(
      num_points=util.map_tree(
          lambda p: (  # pylint: disable=g-long-lambda
              int(np.prod(hmc_state.target_log_prob.shape))
              * jnp.array(scale_smoothing_steps, p.dtype)
          ),
          running_var_state.num_points,
      ),
      mean=util.map_tree(  # pylint: disable=g-long-lambda
          lambda m, init_m: jnp.ones_like(m) * init_m,
          running_var_state.mean,
          initial_mean,
      ),
      variance=util.map_tree(  # pylint: disable=g-long-lambda
          lambda v, init_s: jnp.ones_like(v) * init_s**2,
          running_var_state.variance,
          initial_scale,
      ),
  )
...
def adaptive_hamiltonian_monte_carlo_step(
    ...
    target_log_prob_fn: fun_mc.PotentialFn,
    num_adaptation_steps: Optional[fun_mc.IntArray],
    variance_window_steps: int | fun_mc.IntArray = 100,
    trajectory_length_factor: float | fun_mc.FloatArray = 1.0,
    num_trajectory_ramp_steps: Optional[int] = 100,
    trajectory_warmup_power: float | fun_mc.FloatArray = 1.0,
    max_num_leapfrog_steps: Optional[int] = 100,
    step_size_adaptation_rate: float | fun_mc.FloatArray = 0.05,
    step_size_adaptation_rate_decay_power: float | fun_mc.FloatArray = 0.1,
    target_accept_prob: float | fun_mc.FloatArray = 0.8,
    step_size_averaging_window_steps: int | fun_mc.IntArray = 100,
    jitter_sample_fn: Callable[
        [fun_mc.IntArray, fun_mc.IntArray, Any], fun_mc.IntArray
    ] = (_uniform_jitter),
    log_accept_ratio_reduce_fn: Callable[
        [fun_mc.FloatArray], fun_mc.FloatArray
    ] = (tfp.math.reduce_logmeanexp),
  ...
  state, _ = transform_fn(
      *map(lambda e: jnp.zeros([num_chains] + list(e)), model.event_shape))
  ...
  _, (state_chain, is_accepted_chain, step_size_chain) = jax.jit(
      lambda: fun_mc.trace(
  ...
  # Compute diagnostics.
  accept_rate = jnp.mean(jnp.array(is_accepted_chain, jnp.float32))
  ...
  trajectory_length_factor = jnp.asarray(trajectory_length_factor, dtype=dtype)
  trajectory_warmup_power = jnp.asarray(trajectory_warmup_power, dtype=dtype)
  ...
  # estimate.
  scale = util.map_tree(jnp.sqrt, running_var_state.variance)
  step_size = ssa_state.step_size(num_adaptation_steps=num_adaptation_steps)
  num_leapfrog_steps = jnp.array(
      jnp.ceil(trajectory_length_factor / step_size), jnp.int32
  )
  num_leapfrog_steps = jnp.maximum(1, num_leapfrog_steps)
  if max_num_leapfrog_steps is not None:
    num_leapfrog_steps = jnp.minimum(max_num_leapfrog_steps, num_leapfrog_steps)
  ...
  jittered_num_leapfrog_steps = jitter_sample_fn(
      num_leapfrog_steps, step, jitter_seed
  )
  ...
  if num_adaptation_steps is not None:
    ...
    running_var_state = util.map_tree(
        lambda n, o: jnp.where(step < num_adaptation_steps, n, o),  # pylint: disable=g-long-lambda
        running_var_state,
...
def interactive_trace(
    ...
    num_steps: fun_mc.IntArray,
    trace_mask: bool | fun_mc.BooleanNest = True,
    iteration_axis: int = 0,
    block_until_ready: bool = True,
    progress_bar_fn: Callable[[Iterable[Any]], Iterable[Any]] = (
        _tqdm_progress_bar_fn
    ),
) -> tuple[fun_mc.State, fun_mc.ArrayNest]:
...
def persistent_hamiltonian_monte_carlo_init(
    ...
    init_level: float | fun_mc.FloatArray = 0.0,
  ...
  return PersistentHamiltonianMonteCarloState(
      ...
      else util.map_tree(jnp.zeros_like, state),
      target_log_prob=target_log_prob,
      state_extra=state_extra,
      direction=jnp.ones_like(target_log_prob),
...
def persistent_hamiltonian_monte_carlo_step(
    ...
    noise_fraction: Optional[fun_mc.FloatArray] = None,
    mh_drift: Optional[fun_mc.FloatArray] = None,
    ...
    integrator_fn: Optional[
        Callable[
            [fun_mc.IntegratorState, fun_mc.FloatArray],
            tuple[fun_mc.IntegratorState, fun_mc.IntegratorExtras],
        ]
    ] = None,
    unroll_integrator: bool = False,
    max_num_integrator_steps: Optional[fun_mc.IntArray] = None,
  ...
  if momentum_sample_fn is None:
    ...
    def _momentum_sample_fn(
        old_momentum: fun_mc.State, seed: Any
    ) -> tuple[fun_mc.State, tuple[()]]:
      ...
      new_momentum = util.map_tree_up_to(
          state, _sample_part, old_momentum, seeds, named_axis
      )
      return new_momentum
  ...
  if integrator_fn is None:
    ...
    def _integrator_fn(
        state: fun_mc.IntegratorState, direction: fun_mc.FloatArray
    ) -> tuple[fun_mc.IntegratorState, fun_mc.IntegratorExtras]:
      directional_step_size = util.map_tree(
          lambda step_size, state: (  # pylint: disable=g-long-lambda
              ...
              * jnp.reshape(
                  direction,
                  list(direction.shape)
                  + [1] * (len(state.shape) - len(direction.shape)),
              )
          ),
          step_size,
          state.state,
